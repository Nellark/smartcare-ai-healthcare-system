...
public class PatientDomainService : IPatientDomainService
{
    ...
    public async Task<Result<IReadOnlyList<Domain.Entities.Patient>>> GetPatientsWithOverlappingAppointmentsAsync(DateTime startTime, DateTime endTime, CancellationToken cancellationToken = default)
    {
        if (startTime >= endTime)
            return Result<IReadOnlyList<Domain.Entities.Patient>>.Failure(Error.InvalidInput);
        ...
        return Result<IReadOnlyList<Domain.Entities.Patient>>.Success(patientsWithOverlappingAppointments.AsReadOnly());
    }
    ...
    private static bool HasOverlappingMedicalRecords(Domain.Entities.Patient patient, DateTime startTime, DateTime endTime)
    {
